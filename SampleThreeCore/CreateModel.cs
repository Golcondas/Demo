    ...
    public class CreateModel
    {
        ...
        public string BuildModels(string path, string dbname)
        {
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string basicModel = FileOperator.ReadFile(".\\WebBasic\\{S_tablename}.cs");
        ...
        public string BuildModels(string path, ArrayList tablelist, string dbtype)
        {
            try
            {
                ...
                FileOperator.WriteFile(path + "Model.csproj", FileOperator.ReadFile(path + "Model.csproj").Replace("<!--DHELPERMODELE-->", modelcsproj.ToString() + "<!--DHELPERMODELE-->"));
